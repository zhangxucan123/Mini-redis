...
public class Parser extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf byteBuf, List<Object> list) throws Exception {
        ...
        // 这里直接就把长度读出去了  我们自定义了数据decode的方式 防止粘包现象的出现。
    ...
    private void parseMultiBulkHeader(byte[] msg) {
        ...
        try {
            int expectedLine = Integer.parseInt(new String(numByte, SystemConfig.SYSTEM_CHARSET)); // 将byte数组解析为Integer
            if (expectedLine == 0) {
    ...
    private Reply parseSingleLineReply(byte[] msg) {
        ...
        switch (msg[0]) {
            case '+':
                result = StatusReply.makeStatusReply(str.substring(1));
                break;
            case '-':
                result = CommonErrorReply.makeCommonErrorReply(str.substring(1));
                break;
            case ':':
                try {
